<!-- Source: https://arkadium.ai/manifest/
...
- **Correctesa**: no hi ha manera objectiva de verificar les seves respostes en qüestions humanes (ètica, política, judici). Funcionen on hi ha una «veritat de referència» (matemàtiques, codi); no funcionen on no n'hi ha.
- **Transparència**: el que el model "diu que pensa" no reflecteix el seu càlcul intern. Les seves cadenes de raonament són narratives a posteriori.
- **Generalització**: són composicionalment fràgils. Tornar a plantejar lleugerament el mateix problema els pot fer fracassar.
- **Eficiència**: les lleis d'escala s'alenteixen. Cada nova generació necessita ordres de magnitud més de còmput per millores cada cop menors.
...
L'alternativa és estructural i ja apunta. Els models petits ben organitzats superen els grans en dominis específics; els sistemes capdavanters dels últims anys combinen xarxes neuronals amb capes simbòliques que aporten estructura, verificació i memòria explícita. El que falta per estendre aquest patró del domini matemàtic al domini humà és una ontologia universal. El Meta-Globàlium és **una estructura de les més denses imaginables** — síntesi de la realitat humana sobre pocs eixos primitius, navegable des d'una mirada panoràmica simple fins al detall ultrafí — i proposa el llenguatge intermedi humà–màquina sobre el qual fer-ho possible: transparent, ancorat estructuralment, obert i auditable.
...
- **Mapa global on els punts cecs són explícits**. Una geometria fixa de la realitat permet, per primera vegada, veure el que **no** s'ha pensat: les dimensions tocades per una resposta es marquen sobre el mapa, i les omeses hi queden visibles com a punts cecs. La metacognició estructural deixa de ser una metàfora — esdevé una propietat operativa.
- **Pensar per parells, no per llistes**. Cada concepte es defineix per la seva relació amb el seu oposat — *llibertat ↔ responsabilitat*, *teoria ↔ pràctica*, *individu ↔ col·lectiu*. La tensió entre contraris no és un accident: és la matèria primera de l'enteniment humà.
- **Un criteri intern per al bé**. *El bé com a harmonia entre les parts*: una resposta plena no és la que segueix una norma exterior, sinó la que no col·lapsa cap dimensió rellevant. Aquest criteri es pot calcular automàticament — no cal una llista de regles textuals.
- **La mateixa forma a totes les escales**. 8 categories ofereixen una mirada panoràmica; 80 donen precisió fina; en àmbits especialitzats el patró es desplega fins als milers. Cada nivell repeteix la mateixa geometria — es pot fer *zoom* sense canviar de mapa.
- **Reconeixible pel sentit comú humà**. Els eixos del model — subjecte/objecte, teoria/pràctica, fenomen/noümen, plasma/món — són distincions que qualsevol persona reflexiva reconeix. Tradicions filosòfiques de mig món hi han arribat independentment. Això converteix el model en un *llenguatge comú* entre humans i màquines.
...
Aquesta tensió té un correlat civilitzacional que formulem amb una distinció clarificadora: **globalització no és el mateix que globalística**. La *globalització* tal com l'hem conegut fins ara — extensió del sistema capitalista neoliberal a escala planetària — ha funcionat en realitat com a *colonització uniformitzadora*: unes parts (regions, ideologies, oligopolis tecnològics) imposen les seves realitats a les altres. La *globalística*, lluny d'oposar-s'hi, és precisament la disciplina interdisciplinària que **fa possible una globalització ben feta**: en lloc d'uniformitzar, proveeix un **marc comú d'entesa** sobre el qual cada part pot **regenerar-se respectant les seves diferències** — un element pacificador d'abast universal precisament perquè no demana renúncia a la diversitat sinó la seva articulació estructurada. Per aconseguir-ho fomenta la visió generalista («saber poc de molt») com a complement necessari de l'expertesa estreta («saber molt de poc») que ha alimentat l'oligopoli tecnològic actual. Un agent ancorat al Meta-Globàlium és, en aquest sentit, una eina *globalística* — orientada a oferir un marc comú que permeti la regeneració plural — i no globalitzadora en el sentit uniformitzador.
...
La implementació ja ha començat. El [Metamodeler](/metamodel/) visualitza les categories i les seves relacions; l'Arkadium Framework estructura el cicle d'inferència en quatre fases auditables; **arkadium.ai** n'és la implementació de referència.
