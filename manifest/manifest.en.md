<!-- Source: https://arkadium.ai/manifest/en/
...
Of the possible topologies, the sphere — and, in higher dimensions, the hypersphere — is the one that best fits this function: the simplest of the perfect figures, where every point maintains the same relation to the centre and to its opposite. It works as a *mental wheel*: a simple geometric abstraction that may change artificial reasoning as the physical wheel changed transport. The intuition comes from afar — from Llull's combinatorial wheels to Xirinacs's spherical topology — but only now is it **computationally operative**.

The **Meta-Globàlium** is the geometric model. **Arkadium**, already deployed at [arkadium.ai](https://arkadium.ai), is the first AI anchored to it. This manifesto sets out the proposal, its arguments, and the collective effort it calls for.
...
At the technical frontier, the leading systems of the last few years — those that have reached mathematical olympiad level, the most advanced document retrieval systems, the world models — share a pattern: **they combine a neural network with a symbolic layer that provides structure, verification, and explicit memory**. The purely neural path has hit its ceiling.
...
The alternative is structural and already emerging. Well-organised small models outperform big ones in specific domains; the leading systems of recent years combine neural networks with symbolic layers that provide structure, verification and explicit memory. What is missing to extend this pattern from the mathematical domain to the human domain is a universal ontology. The Meta-Globàlium is **one of the densest structures imaginable** — a synthesis of human reality over a few primitive axes, navigable from a simple panoramic view to ultra-fine detail — and proposes the intermediate human–machine language on which to make it possible: transparent, structurally anchored, open and auditable.
...
- **The same form at every scale**. 8 categories offer a panoramic view; 80 give fine precision; in specialised domains the pattern unfolds into thousands. Each level repeats the same geometry — one can *zoom in* without changing the map.
...
This tension has a civilisational counterpart that we capture with a clarifying distinction: **globalisation is not the same as globalistics**. *Globalisation* as we have known it so far — the planetary extension of neoliberal capitalism — has in fact functioned as *uniformising colonisation*: some parts (regions, ideologies, technological oligopolies) impose their realities on others. *Globalistics*, far from opposing globalisation, is precisely the interdisciplinary discipline that **makes a well-done globalisation possible**: instead of uniformising, it provides a **common framework of understanding** on which each part can **regenerate while respecting its differences** — a pacifying element of universal scope precisely because it does not demand renunciation of diversity but its structured articulation. To this end it fosters the generalist vision ("knowing a little of much") as a necessary complement to the narrow expertise ("knowing much of little") that has fed the current technological oligopoly. An agent anchored to the Meta-Globàlium is, in this sense, a *globalistic* tool — oriented to provide a common framework that enables plural regeneration — and not a globalising one in the uniformising sense.
...
Implementation has begun. The [Metamodeler](/metamodel/) visualises the categories and their relations; the Arkadium Framework structures the inference cycle into four auditable phases; **arkadium.ai** is the reference implementation.
...
- **Technical** (code, specification, translations): *pull requests* to the [GitHub repository](https://github.com/opengea/arkadium).
